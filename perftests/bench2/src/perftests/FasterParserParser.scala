...
object fastparseParser{
  ...
  def obj[$: P]: P[Expr] = P( (Index ~~ objinside).map(Expr.Obj.apply.tupled) )
  ...
  def ifElse[$: P](index: Int): P[Expr] = P( Index ~~ expr ~ "then" ~~ break ~ expr ~ ("else" ~~ break ~ expr).? ).map(Expr.IfElse.apply.tupled)
  def localExpr[$: P]: P[Expr] = P( Index ~~ bind.rep(min=1, sep = ","./) ~ ";" ~ expr ).map(Expr.LocalExpr.apply.tupled)
  ...
  def parened[$: P] = P( (Index ~~ expr).map(Expr.Parened.apply.tupled) )
  ...
  def strExpr[$: P] = P((Index ~~ string).map(Expr.Str.apply.tupled))
  def idExpr[$: P] = P( (Index ~~ id).map(Expr.Id.apply.tupled) )
  ...
  def forspec[$: P] = P( Index ~~ "for" ~~ break ~/ id ~ "in" ~~ break ~ expr ).map(Expr.ForSpec.apply.tupled)
  def ifspec[$: P] = P( Index ~~ "if" ~~ break  ~/ expr ).map(Expr.IfSpec.apply.tupled)
  def fieldname[$: P] = P( id.map(Expr.FieldName.Fixed) | string.map(Expr.FieldName.Fixed) | "[" ~ expr.map(Expr.FieldName.Dyn) ~ "]" )
  def assertStmt[$: P] = P( "assert" ~~ break  ~/ expr ~ (":" ~ expr).? ).map(Expr.Member.AssertStmt.apply.tupled)
  def bind[$: P] = P( Index ~~ id ~ ("(" ~/ params.? ~ ")").?.map(_.flatten) ~ "=" ~ expr ).map(Expr.Bind.apply.tupled)
